  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-2">Lista de clientes suscritos a tu boletín exclusivo.</p>
      </div>

      {subscribers.length === 0 ? (
        <div className="bg-white dark:bg-neutral-950 border border-gray-200 rounded-sm p-12 text-center">
          <p className="text-gray-500">Aún no tienes suscriptores. Aparecerán aquí cuando un cliente se una desde el pie de página.</p>
        </div>
      ) : (
        <div className="bg-white dark:bg-neutral-950 border border-gray-200 rounded-sm overflow-hidden">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-neutral-900 border-b border-gray-200 text-xs uppercase tracking-widest text-gray-500">
            ...
            <tbody className="divide-y divide-gray-200">
              {subscribers.map((sub) => (
                <tr key={sub.id} className="hover:bg-gray-50 dark:hover:bg-neutral-900 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <a href={`mailto:${sub.email}`} className="flex items-center gap-2 text-sm text-gray-500 hover:text-black dark:hover:text-white">
